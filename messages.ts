...
router.get("/", (req, res) => {
  Message.find({})
    .then((messages) => {
      ...
      if (messages.length !== 0) res.status(200).send(messages);
      else res.status(200).json({ message: "No messages found" });
    })
    .catch((error) => {
...
router.delete("/:id", async (req, res) => {
  if (req.params.id === "deleteall") {
    ...
    Message.findByIdAndDelete(req.params.id)
      .then((message) => {
        if (!message) res.status(404).send("No message found");
        else res.status(200).json({ message: "Message deleted successfully" });
      })
      .catch((error) => {
        res.status(500).send("Internal Server Error");
      });
